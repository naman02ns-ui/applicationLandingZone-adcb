...
resource "azurerm_private_endpoint" "pep" {
  ...
  name                = format("pe-sa-%s-%s-%s-%s", var.application_name, var.environment, local.location_shortcode, module.res-id.result)
  location            = local.location
...
resource "azurerm_private_endpoint" "pep-fileshare" {
  ...
  name                = format("pe-sa-fileshare-%s-%s-%s-%s", var.application_name, var.environment, local.location_shortcode, module.res-id.result)
  location            = local.location
  resource_group_name = var.resource_group_name
  subnet_id           = data.azurerm_subnet.privatelink_subnet[0].id
...
resource "azurerm_private_endpoint" "funcapp_pep" {
  for_each = var.privatelink_funcapp_subnet != null ? azurerm_linux_function_app.function-app : {}

  name                = format("pe-func-%s-%s-%s-%s", var.application_name, var.environment, each.key, local.location_shortcode)
  location            = local.location
  resource_group_name = var.resource_group_name
