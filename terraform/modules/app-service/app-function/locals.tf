locals {
  ...
  artifacts = merge(
    ...
    var.application_insights_enabled ? {
      "APPINSIGHTS_INSTRUMENTATIONKEY"         = try(module.app-insights[0].instrumentation_key, null),
      "APPLICATIONINSIGHTS_CONNECTION_STRING"  = try(module.app-insights[0].connection_string, null)
    } : {}
  )
